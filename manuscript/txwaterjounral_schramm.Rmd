...
We modeled the likelihood that a SWQM site would have adequate statistical power as a function of sample size, variance, and effect size using generalized additive models (GAMs). GAMs are a flexible semi-parametric approach in which the response variable is a function of the sum of smoothing functions applied to independent covariates [@wood_fast_2011; @wood_2017]. GAMs were fit for each set of effect sizes with the mgcv package in R [@wood_2017]. The general form of the GAM equation was:

$$
Power = s(cv) + s(sample\ size) + \epsilon
$$
where *s* is a spline based smoothing function applied to each covariate, *cv* is the sample CV, and *sample size* is the annual number of samples collected at the site.  The response variable *Power* is the statistical power within the interval [0,1]. GAMs were fit using the beta regression family and the logit link function. GAMs were not used to specifically test for relationships but only to describe and visualize the known functional relationships that influence statistical power of trend tests as they apply to SWQM stations.
